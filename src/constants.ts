...
export class FactorColumn {
  ...
  constructor(key: HofstedeFactor, name: string, description: string) {
    this.key = key;
    this.name = name;
...
export const defaultFactorValues: HofstedeValues = (Object.fromEntries(
  HOFSTEDE.cols.map((col) => [col.key, Math.ceil(Math.random() * 100)])
) as unknown) as HofstedeValues;
